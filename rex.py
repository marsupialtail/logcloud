...
if mode == "batch":
    ...
    os.mkdir(index_name + "/split_" + str(split))
    os.mkdir(index_name + "/split_" + str(split) + "/parquets")
    ...
    for i in range(0, len(files)):
        ...
        if current_size >= index_interval * 1024 * 1024 or i == len(files) - 1:
            ...
            # run the indexer
            parquet_dir = index_name + "/split_" + str(split) + "/parquets/"
            ...
            if current_size_compaction >= compaction_interval * 1024 * 1024 or i == len(files) - 1:
                ...
                [os.rename(index_name + k, index_name + "/split_" + str(split) + "/" + index_name + k) for k in [".kauai", ".oahu", ".hawaii"]]
