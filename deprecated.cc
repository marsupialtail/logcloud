...
std::string compress_page(const btree_page_t& page, CompressionAlgorithm algorithm = CompressionAlgorithm::ZSTD) {
    ...
    std::string compressed = compressor.compress(reinterpret_cast<const char*>(serialized_page.data()), serialized_page.size() * sizeof(bwt_int));
    return compressed;
}

btree_page_t decompress_page(const std::string& compressed_page, CompressionAlgorithm algorithm = CompressionAlgorithm::ZSTD) {
    Compressor compressor(algorithm);
    std::string decompressed = compressor.decompress(compressed_page);
    const bwt_int* data_ptr = reinterpret_cast<const bwt_int*>(decompressed.data());
...
// an element in a page will not point to the id of the leaf page, but rather the leaf page's compressed byte offset!

std::tuple<btree_t, std::string, size_t> construct_btree( bwt_int * elements, size_t n )
{
    btree_t btree;
    ...
    for (size_t i = 0; i < n; i += B) {
        ...
        leaf_mins.push_back(std::make_tuple(elements[i], compressed_page_offsets[page_count]));
        page_count++;
    }
    ...
    while (leaf_mins.size() > B) {
        ...
        for (size_t i = 0; i < leaf_mins.size(); i += B) {
            ...
            compressed_tree << compress_page(page);
            compressed_page_offsets.push_back(compressed_tree.tellp());
            new_leaf_mins.push_back(std::make_tuple(std::get<0>(leaf_mins[i]), compressed_page_offsets[page_count]));
            page_count++;
        }
    ...
    compressed_tree << compress_page(root_page);

    // need to return the root page offset
    return std::make_tuple(btree, compressed_tree.str(), compressed_page_offsets[page_count - 1]);
}
...
int main(int argc, const char *argv[1]) {
    ...
    if (strcmp(mode, "index") == 0) {
        ...
        for(int i = 0; i < ALPHABET; i ++) {
            ...
            // }
            // printf("\n");
            auto result = construct_btree(&FM_index[i][0], FM_index[i].size());
            auto btree = std::get<0>(result);
            auto compressed_tree = std::get<1>(result);
            auto root_page_offset = std::get<2>(result);
            total_bytes += compressed_tree.size();
            btrees.push_back(btree);
            
            // std::cout << "btree: " << btree << std::endl;
            // std::cout << "lookup: " << lookup(btree, btree.size() - 1, 0) << std::endl;
        }
        std::cout << "compressed size will be: " << total_bytes << std::endl;
