...
typedef std::vector<fm_chunk> wavelet_tree_t;
...
std::string serializeMap(const std::map<char, size_t>& map) {
    ...
    if (serializedString.size() > 0){serializedString.pop_back();}  // Remove the trailing comma
...
std::string serializeChunk(const fm_chunk& chunk) {
    ...
    // compress it 
    std::string compressed_map = compressor.compress(serialized_map.c_str(), serialized_map.size());
    size_t compressed_map_size = compressed_map.size();
    // we are going to write out this chunk as length of compressed map, compressed map, compressed string
    std::string compressed_chunk = compressor.compress(std::get<1>(chunk).c_str(), std::get<1>(chunk).size());

    std::string serialized_chunk;
    serialized_chunk.resize(sizeof(size_t) + compressed_map_size + compressed_chunk.size());
    memcpy((void *)serialized_chunk.data(), &compressed_map_size, sizeof(size_t));
    memcpy((void *)(serialized_chunk.data() + sizeof(size_t)), compressed_map.data(), compressed_map_size);
    memcpy((void *)(serialized_chunk.data() + sizeof(size_t) + compressed_map_size), compressed_chunk.data(), compressed_chunk.size());
...
fm_chunk deserializeChunk(const std::string& serializedChunk) {
    ...
    std::string compressed_map(serializedChunk.data() + sizeof(size_t), compressed_map_size);
    std::string decompressed_map = compressor.decompress(compressed_map);
    std::map<char, size_t> map = deserializeMap(decompressed_map);
    std::string compressed_chunk(serializedChunk.data() + sizeof(size_t) + compressed_map_size, serializedChunk.size() - sizeof(size_t) - compressed_map_size);
...
std::tuple<size_t, std::vector<size_t>, std::vector<size_t>> read_metadata_from_file(VirtualFileRegion * vfr) {
    ...
    Compressor compressor(CompressionAlgorithm::ZSTD);
    std::string compressed_offsets(buffer.begin(), buffer.begin() + compressed_C_byte_offset - compressed_offsets_byte_offset);
...
wavelet_tree_t construct_wavelet_tree(const char *P, size_t Psize) {
    ...
    for(int idx = 0; idx < Psize; idx++) {
        ...
        if (curr_chunk.size() == CHUNK_CHARS) {
            std::tuple<std::map<char, size_t>, std::string> fm_chunk = std::make_tuple(current_chunk_char_counts, curr_chunk);
