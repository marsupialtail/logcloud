...
arrow::Status compress_logs(std::vector<std::string> &files,
    ...
    while (std::getline(infile, line)) {
      ...
      if (!isValidTimestamp(epoch_ts)) {
        if (last_timestamp == 0) {
          LOG(WARNING)
              ...
              << std::endl;
          LOG(WARNING) << "this will lead to wrong extracted timestamps"
                       << std::endl;
      ...
      for (const std::string &token : tokens) {
        ...
        if (inverted_index[type_of_token][token].size() == 0 ||
          ...
          inverted_index[type_of_token][token].push_back(current_line_number /
                                                         ROW_GROUP_SIZE);
        }
      }
  ...
  for (auto const &[t, items] : inverted_index) {
    if (items.size() > OUTLIER_THRESHOLD) {
      std::ofstream compacted_type_file("compressed/" +
