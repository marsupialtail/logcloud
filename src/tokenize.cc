	...
	for (size_t f = 0; f < files.size(); ++f) {
		...
		while (std::getline(infile, line)) {
			...
			if (!isValidTimestamp(epoch_ts)) {
				if (last_timestamp == 0) {
					LOG(WARNING) << "unable to backfill timestamp for a log "
									"line, most likely "
									"because the start of a file does not "
									"contain valid timestamp"
								 << std::endl;
					LOG(WARNING)
						<< "this will lead to wrong extracted timestamps"
						<< std::endl;
					LOG(WARNING) << "attempted to parse "
			...
			for (const std::string &token : tokens) {
				...
				if (inverted_index.find(type_of_token) ==
					inverted_index.end()) {
					inverted_index[type_of_token] = {};
				}
				if (inverted_index[type_of_token][token].size() == 0 ||
					inverted_index[type_of_token][token].back() !=
						current_line_number / ROW_GROUP_SIZE) {
					inverted_index[type_of_token][token].push_back(
						current_line_number / ROW_GROUP_SIZE);
				}
			}
