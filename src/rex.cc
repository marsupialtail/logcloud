...
// feed the LogCrisp compressor chunks of 64MB. Since this is the only size it's
// been comprehensively tested at!
...
std::tuple<std::map<int, std::set<variable_t>>,
		   std::map<int, std::vector<variable_t>>>
get_variable_info(int total_chunks, size_t group_number) {
	...
	for (int chunk = 0; chunk < total_chunks; ++chunk) {
		std::string variable_tag_file =
			"compressed/" + std::to_string(group_number) + "/variable_" +
			std::to_string(chunk) + "_tag.txt";
...
extern "C" int compressor_wrapper(std::string &chunk, std::string output_path,
								  std::string template_path, int prefix);

void compress_chunk(int chunk_file_counter, std::string &current_chunk,
					std::string template_name, size_t group_number) {
	std::string dirName = "variable_" + std::to_string(chunk_file_counter);
	std::string tagName =
		"variable_tag_" + std::to_string(chunk_file_counter) + ".txt";
	...
	oss << "compressed/" + std::to_string(group_number) + "/chunk"
		<< std::setw(4) << std::setfill('0') << chunk_file_counter;
	std::string chunk_filename = oss.str();
	compressor_wrapper(current_chunk, chunk_filename, template_name,
					   chunk_file_counter);

	std::filesystem::rename("variable_" + std::to_string(chunk_file_counter),
							"compressed/" + std::to_string(group_number) +
								"/variable_" +
								std::to_string(chunk_file_counter));
	std::filesystem::rename(
		"variable_" + std::to_string(chunk_file_counter) + "_tag.txt",
		"compressed/" + std::to_string(group_number) + "/variable_" +
			std::to_string(chunk_file_counter) + "_tag.txt");
}
...
arrow::Status write_parquet_file(std::string parquet_files_prefix,
								 std::shared_ptr<arrow::Table> &table) {

	// what we are going to do here is figure out first the last parquet file
	// written, which could be None then we are going to do a copy on write
	// strategy to make sure that the all the parquet files except the last one
	// are full
	...
	while (fs::exists(parquet_files_prefix +
					  std::to_string(parquet_file_counter) + ".parquet")) {
		parquet_file_counter++;
	}
	if (parquet_file_counter > 0) {
		parquet_file_counter--;
		std::string filename = parquet_files_prefix +
							   std::to_string(parquet_file_counter) +
							   ".parquet";
		...
		ARROW_ASSIGN_OR_RAISE(table,
							  arrow::ConcatenateTables({last_table, table}));
	}

	while (table->num_rows() >= ROW_GROUP_SIZE * ROW_GROUPS_PER_FILE) {
		// Write the table to a parquet file
		size_t write_lines = ROW_GROUP_SIZE * ROW_GROUPS_PER_FILE;
		std::string parquet_filename = parquet_files_prefix +
									   std::to_string(parquet_file_counter++) +
									   ".parquet";
		std::shared_ptr<arrow::io::FileOutputStream> outfile;
		ARROW_ASSIGN_OR_RAISE(
			outfile, arrow::io::FileOutputStream::Open(parquet_filename));
		ARROW_RETURN_NOT_OK(parquet::arrow::WriteTable(
			*(table->Slice(0, write_lines)), arrow::default_memory_pool(),
			outfile, ROW_GROUP_SIZE, writer_properties));
...
arrow::Status compress_logs(std::vector<std::string> &files,
							std::string &index_name, size_t group_number,
							size_t timestamp_bytes,
							std::string &timestamp_format,
							std::string &parquet_files_prefix) {

	std::string template_prefix =
		"compressed/" + index_name + "_" + std::to_string(group_number);
	...
	std::shared_ptr<arrow::Schema> schema =
		arrow::schema({arrow::field("timestamp", arrow::uint64()),
	...
	for (size_t f = 0; f < files.size(); ++f) {
		...
		// use a do while loop here else you are going to miss the last line.
		// ChatGPT is still not smart enough.
		...
		while (std::getline(infile, line)) {
			...
			std::string extract_timestamp_from_this_line =
				line.substr(0, timestamp_bytes);
			strptime(extract_timestamp_from_this_line.c_str(),
					 timestamp_format.c_str(), &tm);
			// convert tm into epoch time in seconds
			size_t epoch_ts = mktime(&tm);

			if (!isValidTimestamp(epoch_ts)) {
				if (last_timestamp == 0) {
					std::cout << "unable to backfill timestamp for a log line, "
								 "most likely "
								 "because the start of a file does not contain "
								 "valid timestamp"
							  << std::endl;
					std::cout << "this will lead to wrong extracted timestamps"
							  << std::endl;
					std::cout << "attempted to parse "
							  << extract_timestamp_from_this_line << " with "
							  << timestamp_format << std::endl;
				}
			...
			if (current_chunk.size() >= CHUNK_SIZE) {
				...
				ARROW_RETURN_NOT_OK(
					timestamp_builder.AppendValues(epoch_ts_vector));
				...
				std::shared_ptr<arrow::Table> table =
					arrow::Table::Make(schema, {timestamp_array, log_array});
				...
				arrow::Status s =
					write_parquet_file(parquet_files_prefix, table);
				if (!s.ok()) {
					return s;
				}
	...
	Now go process the compressed chunks, extract using LogCrisp and produce
	variables

	*/

	size_t total_chunks = chunks.size();
	auto [chunk_variables, eid_to_variables] =
		get_variable_info(total_chunks, group_number);
	...
	if (group_number == 0) {
		...
		std::ifstream current_line_number_file(
			"compressed/" + std::to_string(group_number - 1) +
			"/current_line_number");
		if (!current_line_number_file.is_open()) {
			return arrow::Status::ExecutionError(
				"Failed to open current_line_number file");
		}
		std::string line;
	...
	for (int chunk = 0; chunk < total_chunks; ++chunk) {
		...
		for (const auto &variable : chunk_variables[chunk]) {
			variable_files[variable] = new std::ifstream(
				"compressed/" + std::to_string(group_number) + "/variable_" +
				std::to_string(chunk) + "/E" + std::to_string(variable.first) +
				"_V" + std::to_string(variable.second));
		}

		std::ostringstream oss;
		oss << "compressed/" + std::to_string(group_number) + "/chunk"
			<< std::setw(4) << std::setfill('0') << chunk << ".eid";
		std::string chunk_filename = oss.str();
		...
		if (!eid_file.is_open()) {
			...
			return arrow::Status::ExecutionError("Failed to open " +
												 chunk_filename);
		}
		...
		for (int eid : chunk_eids) {
			...
			//     std::cout << "current_line_number: " << current_line_number
			//     << std::endl;
			//     // compress and write the variable_buffer to disk
			//     Compressor compressor(CompressionAlgorithm::ZSTD);
			//     std::string compressed_str =
			//     compressor.compress(variable_buffer.c_str(),
			//     variable_buffer.size()); fwrite(compressed_str.c_str(),
			//     sizeof(char), compressed_str.size(), fp);
			//     byte_offsets.push_back(ftell(fp));
			//     variable_buffer = "";
			// }

			if ((eid < 0) ||
				(eid_to_variables.find(eid) == eid_to_variables.end())) {
				// this is an outlier or if this templates does not have
				// variables, skip it
				current_line_number++;
				continue;
			}
				...
				for (const auto &variable : this_variables) {
					...
					// IMPORTANT: we will use the actual type of the object here
					// instead of the type of the LogGrep variable this is to
					// reduce dependency on the correctness of LogGrep and speed
					// up exact match since with exact match you can now search
					// just one type instead of all possible super-types
					int t = get_type(item.c_str());
					if (t == 0) {
						std::cerr
							<< "WARNING, null variable detected in LogCrisp."
							<< chunk << " " << variable.first << " "
							<< variable.second
							<< "This variable is not indexed." << std::endl;
						continue;
					}
				...
				for (const auto &entry : type_vars) {
					...
					expanded_items[t].insert(expanded_items[t].end(),
											 entry.second.begin(),
											 entry.second.end());
					expanded_lineno[t].resize(
						expanded_lineno[t].size() + entry.second.size(),
						current_line_number / ROW_GROUP_SIZE);
				}
				current_line_number++;
			}
		}
	...
	std::ofstream current_line_number_file(
		"compressed/" + std::to_string(group_number) + "/current_line_number");
	...
	std::ofstream outlier_file("compressed/" + std::to_string(group_number) +
							   "/outlier");
	std::ofstream outlier_lineno_file(
		"compressed/" + std::to_string(group_number) + "/outlier_lineno");
	std::vector<std::string> outlier_items;
	std::vector<std::vector<size_t>> outlier_lineno;

	for (const int &t : touched_types) {
		if (expanded_items[t].empty()) {
			return arrow::Status::ExecutionError(
		...
		if (compacted_items.size() > OUTLIER_THRESHOLD) {
			if (compacted_type_files[t] == nullptr) {
				compacted_type_files[t] = new std::ofstream(
					"compressed/" + std::to_string(group_number) +
					"/compacted_type_" + std::to_string(t));
				compacted_lineno_files[t] = new std::ofstream(
					"compressed/" + std::to_string(group_number) +
					"/compacted_type_" + std::to_string(t) + "_lineno");
			}
			...
			//     outlier_lineno_file << "\n";
			// }
			outlier_items.insert(outlier_items.end(), compacted_items.begin(),
								 compacted_items.end());
			outlier_lineno.insert(outlier_lineno.end(),
								  compacted_lineno.begin(),
								  compacted_lineno.end());
		}
	...
	//     Compressor compressor(CompressionAlgorithm::ZSTD);
	//     std::string compressed_str =
	//     compressor.compress(variable_buffer.c_str(), variable_buffer.size());
	//     fwrite(compressed_str.c_str(), sizeof(char), compressed_str.size(),
	//     fp); variable_buffer = "";
	// }
	// fclose(fp);
...
void rex_python(size_t number_of_files, const char **filenames,
				const char *index_name, size_t group_number,
				size_t timestamp_bytes, const char *timestamp_format,
				const char *parquet_files_prefix) {
	...
	arrow::Status s =
		compress_logs(files, indexNameStr, group_number, timestamp_bytes,
					  timestampFormatStr, parquetFilesPrefixStr);
	if (!s.ok()) {
		std::cout << s.ToString() << "\n";
	}
}
...
int main(int argc, char *argv[]) {
	if (argc < 6) {
		std::cerr << "Usage: " << argv[0]
				  << " <input_filenames> <output_template_base> <group_number> "
					 "<timestamp_bytes> <timestamp_prefix>\n";
		return -1;
	}
	...
	arrow::Status s =
		compress_logs(files, index_name, group_number, timestamp_bytes,
					  timestamp_format, parquet_files_prefix);
