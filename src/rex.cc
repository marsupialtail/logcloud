...
void compress_chunk(int chunk_file_counter, std::string &current_chunk,
  ...
  oss << "compressed/" + std::to_string(group_number) + "/chunk" << std::setw(4)
      << std::setfill('0') << chunk_file_counter;
...
arrow::Status write_parquet_file(std::string parquet_files_prefix,
  ...
  while (table->num_rows() >= ROW_GROUP_SIZE * ROW_GROUPS_PER_FILE) {
    ...
    ARROW_RETURN_NOT_OK(parquet::arrow::WriteTable(
        *(table->Slice(0, write_lines)), arrow::default_memory_pool(), outfile,
...
arrow::Status compress_logs(std::vector<std::string> &files,
  ...
  for (size_t f = 0; f < files.size(); ++f) {
    ...
    while (std::getline(infile, line)) {
      ...
      if (!isValidTimestamp(epoch_ts)) {
        if (last_timestamp == 0) {
          std::cout
              << "unable to backfill timestamp for a log line, most likely "
                 "because the start of a file does not contain valid timestamp"
              << std::endl;
          std::cout << "this will lead to wrong extracted timestamps"
                    << std::endl;
          std::cout << "attempted to parse " << extract_timestamp_from_this_line
                    << " with " << timestamp_format << std::endl;
        }
  ...
  */
  ...
  if (group_number == 0) {
    ...
    std::ifstream current_line_number_file("compressed/" +
                                           std::to_string(group_number - 1) +
                                           "/current_line_number");
  ...
  for (int chunk = 0; chunk < total_chunks; ++chunk) {
    ...
    for (const auto &variable : chunk_variables[chunk]) {
      variable_files[variable] = new std::ifstream(
          "compressed/" + std::to_string(group_number) + "/variable_" +
          std::to_string(chunk) + "/E" + std::to_string(variable.first) + "_V" +
    ...
    for (int eid : chunk_eids) {
      ...
      if ((eid < 0) || (eid_to_variables.find(eid) == eid_to_variables.end())) {
        // this is an outlier or if this templates does not have variables, skip
        // it
        current_line_number++;
        continue;
      }
        ...
        for (const auto &variable : this_variables) {
          ...
          // IMPORTANT: we will use the actual type of the object here instead
          // of the type of the LogGrep variable this is to reduce dependency on
          // the correctness of LogGrep and speed up exact match since with
          // exact match you can now search just one type instead of all
          // possible super-types
          int t = get_type(item.c_str());
          if (t == 0) {
            std::cerr << "WARNING, null variable detected in LogCrisp." << chunk
        ...
        for (const auto &entry : type_vars) {
          ...
          expanded_lineno[t].resize(expanded_lineno[t].size() +
                                        entry.second.size(),
  ...
  for (const int &t : touched_types) {
    ...
    if (compacted_items.size() > OUTLIER_THRESHOLD) {
      if (compacted_type_files[t] == nullptr) {
        compacted_type_files[t] =
            new std::ofstream("compressed/" + std::to_string(group_number) +
                              "/compacted_type_" + std::to_string(t));
        compacted_lineno_files[t] = new std::ofstream(
            "compressed/" + std::to_string(group_number) + "/compacted_type_" +
            std::to_string(t) + "_lineno");
      }
      ...
      outlier_items.insert(outlier_items.end(), compacted_items.begin(),
                           compacted_items.end());
      outlier_lineno.insert(outlier_lineno.end(), compacted_lineno.begin(),
