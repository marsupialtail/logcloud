...
public:
    ...
    //templateNode * SearchTemplate(std::string * tokens , int length); // the funtion of searching for the suitable template
    int parseTemplate(char* log, SegTag * segArray, int token_size);
    int SearchTemplate(char* logs, SegTag * segArray, int segSize, map<int, VarArray*>& variables, bool extract);
