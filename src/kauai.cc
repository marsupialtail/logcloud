...
search_kauai(VirtualFileRegion *vfr, std::string query, int mode, int k) {
  ...
  // first read in the byte_offsets, which should be the last sizeof(size_t) * 6
  // bytes of the file
  ...
  // read in the outlier type posting lists, first get the file size, the limit
  // is file_size - byte_offsets[7] - 8 * sizeof(size_t)
  ...
  if (matched_row_groups.size() >= k) {
    std::cout << "inexact query for top K satisfied by template and outlier "
              << query << std::endl;
  ...
  if (matched_row_groups.size() >= k) {
    std::cout << "inexact query for top K satisfied by template, outlier and "
                 "outlier types "
              << query << std::endl;
    return std::make_pair(1, matched_row_groups);
  } else {
    std::cout << "inexact query for top K not satisfied by template, outlier "
                 "and outlier types "
              << query << std::endl;
...
int write_kauai(std::string filename, int num_groups) {
  ...
  for (size_t group_number = 0; group_number < num_groups; ++group_number) {
    ...
    std::ifstream template_file("compressed/" + filename + "_" +
                                std::to_string(group_number) + ".templates");
    ...
    while (std::getline(template_file, line)) {
      ...
      if (tokens.size() >= 3) {
        size_t key =
            std::stoul(tokens[0].substr(1)); // Extract and convert the key
        std::string value = "";
    ...
    for (size_t chunk = 0; chunk < total_chunks; ++chunk) {
      ...
      while (std::getline(eid_file, line)) {
        ...
        // TODO: this is a hack. eid could be negative which will manifest as
        // very large unsigned long.
        if (eid > 1000000) {
          std::string item;
          ...
          if (template_posting_lists[idx].size() == 0 ||
              template_posting_lists[idx].back() != lineno / ROW_GROUP_SIZE) {
