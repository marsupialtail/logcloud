...
void mergeFiles(const vector<string> &inputFilenames,
                const vector<string> &inputFilenamesLinenumbers,
                const string &outputFilename,
                const string &outputFilenameLinenumbers,
  ...
  while (any_of(currentLines.begin(), currentLines.end(),
    ...
    filteredCurrentLines.erase(
        remove_if(filteredCurrentLines.begin(), filteredCurrentLines.end(),
                  [](const string &s) { return s.empty(); }),
        filteredCurrentLines.end());
    string it =
        *min_element(filteredCurrentLines.begin(), filteredCurrentLines.end());
...
int compact(int num_groups) {
  ...
  string filename =
      "compressed/" + to_string(num_groups - 1) + "/current_line_number";
  ...
  for (int i = 0; i < num_groups; ++i) {
    if (!fs::exists("compressed/" + to_string(i) + "/outlier")) {
      continue;
    }
    inputFilenames.push_back("compressed/" + to_string(i) + "/outlier");
    inputFilenamesLinenumbers.push_back("compressed/" + to_string(i) +
                                        "/outlier_lineno");
  }
  if (!inputFilenames.empty()) {
    mergeFiles(inputFilenames, inputFilenamesLinenumbers, "compressed/outlier",
               "compressed/outlier_lineno", num_row_groups);
  }
  ...
  for (int type = 1; type <= 63; ++type) {
    ...
    for (int i = 0; i < num_groups; ++i) {
      ...
      string filename =
          "compressed/" + to_string(i) + "/compacted_type_" + to_string(type);
      if (!fs::exists(filename)) {
        continue;
      }

      inputFilenames.push_back("compressed/" + to_string(i) +
                               "/compacted_type_" + to_string(type));
      inputFilenamesLinenumbers.push_back("compressed/" + to_string(i) +
                                          "/compacted_type_" + to_string(type) +
                                          "_lineno");
    }
    ...
    string outputFilenameLinenumbers =
        "compressed/compacted_type_" + to_string(type) + "_lineno";

    mergeFiles(inputFilenames, inputFilenamesLinenumbers, outputFilename,
               outputFilenameLinenumbers, num_row_groups);
  }
