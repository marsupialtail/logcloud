...
void analyze_oahu(std::string filename) {
	...
	for (size_t i = 0; i < num_blocks + 1; ++i) {
		std::cout << "Block " << i << " offset: " << block_offsets[i]
				  << std::endl;
	}
	...
	for (size_t i = 0; i < num_blocks; ++i) {
		...
		for (std::vector<plist_size_t> &plist : plist_data) {
			...
			roaring_offsets.push_back(roaring_offsets.back() +
									  serializedString.size());
		}

		std::string compressedString = compressor.compress(
			totalserializedString.c_str(), totalserializedString.size());
		std::string roaring_offsets_string((char *)roaring_offsets.data(),
										   roaring_offsets.size() *
											   sizeof(size_t));
		std::string compressed_roaring_offsets = compressor.compress(
			roaring_offsets_string.c_str(), roaring_offsets_string.size());
		block_roaring_length =
			compressedString.size() + compressed_roaring_offsets.size();

		// now compress csr_offsets and values back to back and see the size
		std::string csr_offsets_string((char *)csr_offsets.data(),
									   csr_offsets.size() * sizeof(size_t));
		std::string compressed_csr_offsets = compressor.compress(
			csr_offsets_string.c_str(), csr_offsets_string.size());
		std::string values_string((char *)values.data(),
								  values.size() * sizeof(size_t));
		std::string compressed_values =
			compressor.compress(values_string.c_str(), values_string.size());

		std::cout << "Block " << i << " compressed csr offsets length: "
				  << compressed_csr_offsets.size() << std::endl;
		std::cout << "Block " << i
				  << " compressed values length: " << compressed_values.size()
				  << std::endl;

		std::cout << "Block " << i
				  << " compressed strings length: " << compressed_strings_length
				  << std::endl;
		std::cout << "Block " << i
				  << " compressed plist length: " << compressed_plist.size()
				  << std::endl;
		std::cout << "Block " << i
				  << " compressed roaring length: " << block_roaring_length
				  << std::endl;

		total_compressed_strings_length += compressed_strings_length;
		total_compressed_plist_length += compressed_plist.size();
		total_csr_length +=
			compressed_csr_offsets.size() + compressed_values.size();
		total_roaring_length += block_roaring_length;
	}

	std::cout << "Total compressed strings length: "
			  << total_compressed_strings_length << std::endl;
	std::cout << "Total compressed plist length: "
			  << total_compressed_plist_length << std::endl;
	std::cout << "Total compressed roaring length: " << total_roaring_length
			  << std::endl;
	std::cout << "Total compressed csr length: " << total_csr_length
			  << std::endl;
}
...
void analyze_hawaii(std::string filename) {
	...
	for (size_t i = 0; i < num_groups * 2 + 1; ++i) {
		group_offsets.push_back(*reinterpret_cast<const size_t *>(
			decompressed_metadata_page.data() + 2 * sizeof(size_t) +
			3 * num_types * sizeof(size_t) + sizeof(size_t) +
			i * sizeof(size_t)));
		std::cout << "Group " << i << " offset: " << group_offsets[i]
				  << std::endl;
	}
	...
	for (size_t i = 0; i < num_groups * 2; i += 2) {
		...
		std::cout << "Group " << group_id << " wavelet size " << wavelet_size
				  << std::endl;
		std::cout << "Group " << group_id << " logidx size " << logidx_size
				  << std::endl;
	...
	std::cout << "Total wavelet tree size: " << total_wavelet_tree_size
			  << std::endl;
