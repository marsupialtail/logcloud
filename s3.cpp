...
int main(int argc, const char *argv[1])
{
    ...
    Aws::InitAPI(options);
    const Aws::String bucket_name = "cluster-dump"; 
    const Aws::String object_name = "test.btree";
    ...
    for (int i = 0; i < N; i++) {
        ...
        std::cout << "Downloaded data size: " << buffer.str().size() << std::endl;
    }

    // end the timer
    auto end = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    std::cout << "Time taken to get object: " << duration.count() << "milliseconds" << std::endl;

    // if (get_object_outcome.IsSuccess()) {
    //     std::cout << "Successfully retrieved object within the byte range." << std::endl;
    // } else {
    //     std::cout << "Error getting object: " << get_object_outcome.GetError().GetExceptionName() 
