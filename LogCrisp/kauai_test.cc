...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        std::cout << "Usage: " << argv[0] << " index <name> <num_groups> or query <name> <query>" << std::endl;
        return 1;
    }
    ...
    if (mode == "index") {
        ...
        std::pair<int, std::vector<plist_size_t>> result = search_kauai(vfr, query, 1, 1000);
        std::cout << "result: " << result.first << " length:" << result.second.size() << std::endl;
