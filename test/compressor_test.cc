...
int main() {
  ...
  std::string compressedZstd =
      compressorZstd.compress(inputData.c_str(), inputData.size());
  ...
  std::string compressedSnappy =
      compressorSnappy.compress(inputData.c_str(), inputData.size());
  std::string decompressedSnappy =
      compressorSnappy.decompress(compressedSnappy);
  assert(inputData == decompressedSnappy);

  Compressor compressorLZ4(CompressionAlgorithm::LZ4);
  std::string compressedLZ4 =
      compressorLZ4.compress(inputData.c_str(), inputData.size());
