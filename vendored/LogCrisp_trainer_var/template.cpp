...
templateNode::templateNode(int eid, char* log, SegTag* segArray, int token_size){ //build from sketch
    Eid = eid;
...
templateNode::~templateNode(){
    ...
    delete[] templates;
    delete[] templatesTags;
}

//the match function foe match,need length
//clove +-
int templateNode::matchMatch(char* tockens, SegTag* segArray, int tocken_size, int skip){
...
double templateNode::parseMatch(char* log, SegTag *segArray, int token_size){
...
void templateNode::merge(char* log, SegTag * segArray, int token_size){
    for (int i = 0; i < token_size; i++){
